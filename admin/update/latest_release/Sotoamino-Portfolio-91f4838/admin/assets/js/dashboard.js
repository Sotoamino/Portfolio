...
addSocialBtn && addSocialBtn.addEventListener('click', () => {
  console.log('Bouton Ajouter un réseau cliqué');
  popup.style.display = 'block';
});

closePopupBtn && closePopupBtn.addEventListener('click', () => {
  console.log('Bouton Annuler popup cliqué');
  popup.style.display = 'none';
});

// ===== Choix d’un réseau social dans la popup =====
...
socialChoiceButtons.forEach(button => {
  button.addEventListener('click', () => {
    ...
    div.querySelector('.remove-social').addEventListener('click', function () {
      console.log(`Suppression du réseau ${network}`);
      div.remove();
    });
...
// ===== Suppression réseaux sociaux existants =====
document.querySelectorAll('.remove-social').forEach(btn => {
  btn.addEventListener('click', function () {
    console.log('Bouton supprimer réseau cliqué');
    this.parentElement.remove();
  });
});

// ===== Extraction du nom LinkedIn si nécessaire =====
window.extractLinkedin = function(input) {document.addEventListener('DOMContentLoaded', () => {
  console.log("dashboard.js chargé");
  ...
  // ===== Suppression réseaux sociaux via délégation =====
  ...
  if (input.name === 'linkedin') {
    ...
    if (parts.length > 1) {
      input.value = 'https://linkedin.com/in/' + parts[1];
      console.log('Lien LinkedIn corrigé:', input.value);
    }
  }
}
...
window.extractGithub = function(input) {
  if (input.name === 'github') {
    const val = input.value;
    const parts = val.split('com/');
    if (parts.length > 1) {
      input.value = 'https://github.com/' + parts[1];
      console.log('Lien Github corrigé:', input.value);
    }
  }
}
